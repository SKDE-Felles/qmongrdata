  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      div(class = "container", style ="margin-right:(@gutter / 10)" ,
          tags$ul(
            tags$li(h5(tags$b(names(KvalIndData)[1]), ": året for hendelsen")),
            tags$li(h5(tags$b(names(KvalIndData)[2]), ": registerets navn på enheten knyttet til hendelsen")),
            tags$li(h5(tags$b(names(KvalIndData)[3]), ": registerets numeriske ID (ofte RESH) på enheten knyttet til hendelsen")),
            tags$li(h5(tags$b(names(KvalIndData)[4]), ": organisasjonsnummeret til sykehus/HF knyttet til hendelsen,
                       hentet fra tilsendt fil SykehusNavnStruktur")),
            tags$li(h5(tags$b(names(KvalIndData)[5]), ": enten en binær indikator (0/1) for fremstilling av andel måloppnåelse,
                       eller en numerisk variabel for fremstilling av gjennomsnitt eller median")),
            tags$li(h5(tags$b(names(KvalIndData)[6]), ": identifikator for kvalitetsindikator, skal samsvare med 
...
server <- function(input, output) {
  ...
  observe(
    if (datatester()$test1 & datatester()$test2 & datatester()$test3 & datatester()$test4) {
  ...
  df <- reactive({
    ...
    df <- read.csv(input$file1$datapath,
                   header = TRUE,
                   sep = input$sep,
                   fileEncoding = if (input$tegnsett == 'Annet') {input$andretegn_verdi} else input$tegnsett,
                   stringsAsFactors = FALSE)
    # colClasses = c('integer', 'character', 'integer', 'integer', 'numeric', 'character')
  })
  ...
  datatester <- function() {
    req(df())
    # Test 1: Riktig antall kolonner i opplasting
  ...
  output$feilmeld <- renderUI({
    ...
    tagList(
        tags$ul(
          if (!data_test$test1) {tags$li(h5(tags$b(data_test$test1tekst)))},
          if (!data_test$test2) {tags$li(h5(tags$b(data_test$test2tekst)))},
          if (!data_test$test3) {tags$li(h5(tags$b(data_test$test3tekst)))},
          if (!data_test$test4) {tags$li(h5(tags$b(data_test$test4tekst)))}
        )
    )
  })
