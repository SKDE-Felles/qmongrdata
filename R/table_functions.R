...
app_data <- function(config) {
  register_data <- qmongrdata::load_data()
  grouped_by_hf <- qmongrdata::group_data(
    register_data,
  ...
  grouped_by_rhf <- qmongrdata::group_data(
    register_data,
  ...
  grouped_by_hospital <- qmongrdata::group_data(
    register_data,
    by = "hospital"
  ) %>%
    dplyr::left_join(
    ...
    dplyr::filter(!is.na(.data[[config$data$column$unit_name$sh]]))
  national_data <- qmongrdata::group_data(
    register_data,
    by = ""
  )
  return(
